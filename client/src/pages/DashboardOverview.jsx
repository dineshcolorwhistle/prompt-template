        ...
        const fetchStats = async () => {
            try {
                if (userInfo.role === 'Expert') {
                    ...
                    setStats([
                        { label: 'Total Templates', value: data.total || 0, change: 'All time', icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-100', gradient: 'from-indigo-500 to-indigo-600' },
                        { label: 'Approved Templates', value: data.approved || 0, change: 'Live', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-100', gradient: 'from-emerald-500 to-emerald-600' },
                        { label: 'Pending Templates', value: data.pending || 0, change: 'Awaiting', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-100', gradient: 'from-amber-500 to-amber-600' },
                        { label: 'Copy Prompt Count', value: data.copyPromptCount || 0, change: 'Total', icon: Copy, color: 'text-blue-600', bg: 'bg-blue-100', gradient: 'from-blue-500 to-blue-600' },
                    ]);
                    ...
                    if (data.stats) {
                        setStats([
                            { label: 'Active Users', value: data.stats.activeUsers || 0, change: 'Total', icon: Users, color: 'text-indigo-600', bg: 'bg-indigo-100', gradient: 'from-indigo-500 to-indigo-600' },
                            { label: 'Total Templates', value: data.stats.templates || 0, change: 'All time', icon: FileText, color: 'text-emerald-600', bg: 'bg-emerald-100', gradient: 'from-emerald-500 to-emerald-600' },
                            { label: 'Pending Approval', value: data.stats.pendingTemplates || 0, change: 'Waiting', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-100', gradient: 'from-amber-500 to-amber-600' },
                            { label: 'Prompt Copies', value: data.stats.promptCopied || 0, change: 'Total', icon: TrendingUp, color: 'text-blue-600', bg: 'bg-blue-100', gradient: 'from-blue-500 to-blue-600' },
                        ]);
                    }
                    if (data.chartData) {
    ...
    const renderExpertRecentActions = () => (
        ...
        >
            <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Recently Created Templates</h3>
            </div>
            <div className="space-y-3">
                {recentActions.length > 0 ? recentActions.map((template, i) => (
                    <motion.div
                        ...
                        className="flex items-start gap-4 p-4 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer border border-gray-50 hover:border-gray-100"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                                <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${template.status === 'Approved' ? 'bg-emerald-50 text-emerald-700' :
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Your recent templates will appear here</p>
                    </div>
                )}
            </div>
        </motion.div>
                ...
                >
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900">Template Trends</h3>
                        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
                <div className="flex items-center gap-2 mb-5">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900">Recent Template Updates</h3>
                </div>
                <div className="space-y-3">
                    {recentActions.length > 0 ? recentActions.map((action, i) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 mt-0.5">
                                    By: {action.user?.name || 'Unknown'} • <span className={`font-medium ${action.status === 'Approved' ? 'text-emerald-600' :
                                        action.status === 'Pending' ? 'text-amber-600' :
                                            'text-gray-500'
                                        }`}>{action.status}</span>
                                </p>
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">No recent template updates</p>
                    )}
                </div>
            </motion.div>
                    ...
                    {chartData?.recentUsers && chartData.recentUsers.length > 0 ? chartData.recentUsers.map((user, i) => (
                        ...
                        >
                            ...
                            <div className="text-right flex-shrink-0">
                                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${user.role === 'Admin' ? 'bg-purple-50 text-purple-700' :
                                    user.role === 'Expert' ? 'bg-indigo-50 text-indigo-700' :
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">No recent user registrations</p>
                    )}
                </div>
            </motion.div>
        </div>
            ...
            >
                {stats.map((stat, index) => (
                    <motion.div
                        ...
                        className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer relative overflow-hidden group"
                    >
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${stat.change.includes('+') ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                {stat.change}
                            </span>
                        </div>
                        <h3 className="text-gray-500 text-sm font-medium">{stat.label}</h3>
                        <p className="text-3xl font-bold text-gray-900 mt-1 tracking-tight">{stat.value.toLocaleString()}</p>
                    </motion.div>
                ))}
            </motion.div>
