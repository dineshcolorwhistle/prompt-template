...
const OutputFormats = () => {
    ...
    const fetchOutputFormats = async () => {
        ...
        setLoading(true);
        try {
            let url = `${import.meta.env.VITE_API_URL}/api/output-formats?page=${page}&limit=10&`;
            ...
            if (data.result) {
                setOutputFormats(data.result);
                setTotalPages(data.pages);
                setTotalItems(data.total);
            } else {
                setOutputFormats(Array.isArray(data) ? data : []);
            }
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = currentFormat
                ? `${import.meta.env.VITE_API_URL}/api/output-formats/${currentFormat._id}`
                : `${import.meta.env.VITE_API_URL}/api/output-formats`;

            const method = currentFormat ? 'PUT' : 'POST';
            ...
            toast.success(currentFormat ? 'Output format updated successfully' : 'Output format created successfully');
            handleCloseModal();
            fetchOutputFormats();
        } catch (err) {
    ...
    const initiateAction = (id, name, isActive, action) => {
        if (action === 'deactivate') {
            setConfirmModal({
                ...
                name,
                action: 'deactivate',
                title: 'Deactivate Output Format',
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (confirmModal.action === 'deactivate') {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/api/output-formats/${confirmModal.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${userInfo.token}`
                ...
                toast.success('Output format deactivated successfully');
                ...
                toast.success('Output format deleted permanently');
            }
    ...
    return (
        <div className="space-y-6 relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Output Formats</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage output formats for the platform.</p>
                </div>
                <button
            ...
            <div className="flex flex-col sm:flex-row gap-4 bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search output formats by name or slug..."
                        value={searchTerm}
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="font-medium text-gray-900 dark:text-white">No output formats found</p>
                                ...
                                >
                                    <td className="px-6 py-4">
                                        <span className="font-medium text-gray-900 dark:text-white block">{item.name || item.title || "Unnamed Output Format"}</span>
            ...
            {/* Pagination */}
            {!loading && outputFormats.length > 0 && (
                <Pagination
                    currentPage={page}
                        ...
                        >
                            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
                                <h3 className="font-bold text-gray-900 dark:text-white text-lg">
                                    {currentFormat ? 'Edit Output Format' : 'Add New Output Format'}
                                </h3>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 space-y-5">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                                        Output Format Name <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        name="name"
                                ...
                                <div className="pt-2 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-800 mt-2">
                                    ...
                                    >
                                        {formLoading ? (
                                            ...
                                            <><CheckCircle size={16} /> {currentFormat ? 'Save Changes' : 'Create Format'}</>
                                        )}
                                    </button>
                                </div>
                            </form>
