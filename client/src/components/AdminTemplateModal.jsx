...
const AdminTemplateModal = ({ isOpen, onClose, template, onSave }) => {
    ...
    const fetchIndustries = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/industries?status=active&limit=1000`, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
    ...
    const fetchCategories = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/categories?status=active&limit=1000`, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            const data = await res.json();
